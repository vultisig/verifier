...
func (t *DKLSTssService) processQcInbound(handle Handle,
	...
	for {
		...
		for _, message := range messages {
			...
			if err := t.processQcOutbound(handle, sessionID, hexEncryptionKey, parties, localPartyID, isEdDSA); err != nil {
				t.logger.Error("fail to process outbound message", "error", err)
			}
			if isFinished {
