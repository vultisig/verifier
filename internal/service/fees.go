...
func (s *FeeService) ValidateFees(ctx context.Context, req ptypes.PluginKeysignRequest) error {
	...
	if err != nil {
		s.logger.WithError(err).Error("failed to decode tx")
	...
	data := tx.Data
	if len(data) < 4 {
		s.logger.WithError(err).Error("no or too short data in tx")
	...
	if err != nil {
		s.logger.WithError(err).Error("failed to parse ABI")
	...
	if err != nil {
		s.logger.WithError(err).Error("unknown method ID")
	...
	if err := method.Inputs.UnpackIntoMap(args, data[4:]); err != nil {
		s.logger.WithError(err).Error("failed to unpack args")
		return fmt.Errorf("failed get recipient and amount from tx")
	}

	recipient, ok := args["to"].(ecommon.Address)
	if !ok {
		s.logger.Error("to is not an ecommon.Address")
	...
	if strings.TrimPrefix(strings.ToLower(s.feeConfig.USDCAddress), "0x") == strings.TrimPrefix(strings.ToLower(contractAddress), "0x") {
		return fmt.Errorf("usdc address invalid")
	}

	feeInfo, err := s.PublicKeyGetFeeInfo(ctx, req.PublicKey)
	if err != nil {
		s.logger.WithError(err).Error("failed to get fee info")
		return fmt.Errorf("internal error")
	}

	fpc := big.NewInt(int64(feeInfo.FeesPendingCollection))
	amount, ok := args["value"].(*big.Int)
	if !ok {
		s.logger.Error("value is not a *big.Int")
		return fmt.Errorf("invalid amount")
	}

	if fpc.Cmp(amount) != 0 {
		s.logger.Error("fee amount incorrect")
